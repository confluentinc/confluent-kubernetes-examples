...
> **Note**: This example migrates from ZooKeeper to KRaft with **static quorum** (`kraft.version=0`). To migrate to KRaft with **dynamic quorum** (`kraft.version=1`), which allows adding/removing controllers without downtime, see the [Dynamic Quorum ZK-to-KRaft examples](../dynamic-quorum/).
