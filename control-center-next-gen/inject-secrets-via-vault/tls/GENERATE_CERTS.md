# Certificate Generation for Example 2: TLS with syncSecret

This document provides step-by-step instructions to generate self-signed TLS certificates for the TLS with syncSecret example.
...
[dn]
CN = controlcenter-next-gen.vault-example-2.svc.cluster.local
O = Confluent
C = US
...
DNS.2 = controlcenter-next-gen.vault-example-2
DNS.3 = controlcenter-next-gen.vault-example-2.svc
DNS.4 = controlcenter-next-gen.vault-example-2.svc.cluster.local
DNS.5 = *.controlcenter-next-gen.vault-example-2.svc.cluster.local
DNS.6 = localhost
IP.1 = 127.0.0.1
EOF
```
