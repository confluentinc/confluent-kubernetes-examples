...
```bash
kubectl create secret generic prometheus-tls -n operator --from-file=tls.crt=./certs/server.pem --from-file=tls.key=./certs/server-key.pem --from-file=ca.crt=./certs/cacerts.pem
kubectl create secret generic alertmanager-tls -n operator --from-file=tls.crt=./certs/server.pem --from-file=tls.key=./certs/server-key.pem --from-file=ca.crt=./certs/cacerts.pem
```

4. Create client side certs against prometheus & alertmanager
```bash
kubectl create secret generic prometheus-client-tls -n operator --from-file=tls.crt=./certs/server.pem --from-file=tls.key=./certs/server-key.pem --from-file=ca.crt=./certs/cacerts.pem
kubectl create secret generic alertmanager-client-tls -n operator --from-file=tls.crt=./certs/server.pem --from-file=tls.key=./certs/server-key.pem --from-file=ca.crt=./certs/cacerts.pem
```
