# Authentication Swap - SASL/SCRAM to SASL/PLAIN with PLAINTEXT

This example demonstrates a Gateway configuration with authentication swap where clients authenticate using SASL/SCRAM-SHA-256 credentials (`alice/alice-secret`) and the Gateway uses swapped credentials (`bob/bob-secret`) to connect to the Kafka cluster using SASL/PLAIN.
...
4. **Credential Swap**: Gateway retrieves swapped credentials (`bob/bob-secret`) from Vault
5. **JAAS Template Population**: Gateway populates the JAAS template with swapped credentials (`bob`, `bob-secret`)
6. **Kafka Connection**: Gateway connects to Kafka cluster using SASL/PLAIN with swapped credentials
...
- Connection to Kafka with SASL/PLAIN credentials for user `bob`
