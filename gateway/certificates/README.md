...
This guide provides instructions for generating self-signed TLS certificates for CPC Gateway testing and development.

## Overview

The certificates generated by this guide are in **PEM format** and include:
- **CA Certificate** (`cacerts.pem`) - Certificate Authority for signing
- **Gateway Certificate** (`gateway.pem`) - Server certificate for Gateway
- **Private Key** (`privkey.pem`) - Private key for the Gateway certificate
- **Full Chain** (`fullchain.pem`) - Combined Gateway and CA certificates
