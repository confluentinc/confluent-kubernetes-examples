# Authentication Swap - mTLS to SASL/PLAIN with PLAINTEXT as SASL mechanism
...
6. **Kafka Connection**: Gateway connects to Kafka cluster using swapped credentials (`bob/bob-secret`)
...
- Use SASL/PLAIN authentication to Kafka (compatibility with existing Kafka setup)
...
**Important Notes:**
- No SASL configuration is needed - authentication is via client certificate only
...
- The Gateway will swap this identity to `bob` when connecting to Kafka
...
- Connection to Kafka with bob credentials
...
```
kubectl delete secret file-store-config file-store-client-credentials plain-jaas -n confluent
```
