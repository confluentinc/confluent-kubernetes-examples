...
```bash
export TUTORIAL_HOME=<Tutorial directory>/kraft/dynamic-quorum/migration/static-to-dynamic/single-cluster
```

### Migration Flow

This migration is a one-time operation with 4 phases. Phase 1 is MRC-only; single-cluster starts at Phase 2.
...
(skip for single-cluster)    (metadata-level,          (CFK generates                   (pick up new
...
| `pre-setup.sh` | One-time setup: namespace, operator |
| `cleanup.sh` | Phased teardown |
