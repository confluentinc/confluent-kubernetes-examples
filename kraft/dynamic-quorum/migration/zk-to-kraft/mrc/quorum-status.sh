...
echo ""
echo "=== Quorum Status ==="
kafka-metadata-quorum --bootstrap-controller localhost:9074 --command-config /tmp/admin.properties describe --status
echo ""
echo "=== kraft.version ==="
kafka-features --bootstrap-controller localhost:9074 --command-config /tmp/admin.properties describe 2>/dev/null | grep kraft.version
'
